...
struct LCX_PCK
{
	LCXVECTOR3	pck;															// Picking Position or Bound Center
...
struct LCX_AABC
{
	LCXVECTOR3	vcO;															// aabb center
	LCXVECTOR3	vcL;															// half length for x,y and z axis

	LCX_AABC();
	LCX_AABC(const VEC3& _min, const VEC3& _max);

	LCXVECTOR3	Extent() const;
...
struct LCX_OBBC
{
	LCXVECTOR3	vcO;															// obb center
	LCXVECTOR3	vcA[3];															// axis left, top, and depth. they have length.(is not normalized).
};
...
FLOAT LcMath_LineToPlaneKValue(  const LCXPLANE* p1
								 , const VEC3* p2
								 , const VEC3* p3);								// k value = k - (d + n . p1)/( ( p2 - p1) . p )


INT   LcMath_LineToPlaneIntersect(  LCXVECTOR3* Out
								  , const LCXLINE* line
								  , const LCXPLANE* plane);

INT   LcMath_LineToPlaneIntersect(  LCXVECTOR3* Out
								  , const LCXLINE* line
								  , const LCXVECTOR3* plane_pos0
								  , const LCXVECTOR3* plane_normal
								  );
...
INT   LcMath_CuboidToSphere		( const LCX_CUBOID* pHexa
								...
								, FLOAT fRadius=0.0F);							// sphere radius
...
INT   LcMath_CylinderToLine		( LCXVECTOR3* Out								// 원통과 직선의 충돌: Intersection point
								, const LCXCYLINDER* cylinder					// Input Cylinder
								, const LCXLINE* line							// Input Line
								, const LCXVECTOR3* pV3 = NULL);				// Input Line Boundary
